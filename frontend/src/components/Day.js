...
const Day = ({ date, worktimes, bookings, totalDuration, onSelect, worker }) => {
  ...
  useEffect(() => {
    ...
    const initialSlots = calculateAvailableSlots(worktimes, date, totalDuration, bookings)
    //setSlots(initialSlots)

    if (worker && worker !== 'anyone') {
      let filteredSlots = initialSlots.filter(slot => slot.workers.includes(worker))
